...
def ingresoProducto(miProducto):
    ...
    if(miProducto["proCantidad"] > proMaxstock):
        print(
            f"El Producto {proNombre} condigo {proCodigo} se encuentra fuera del rango max stock")

    if(miProducto["proCantidad"] < proMinstock):
        print(
            f"El Producto {proNombre} código {proCodigo} se encuentra fuera del rango min stock")

    if(10 <= miProducto["proCantidad"] <= 500):
        # else:
        print(f"El producto {proNombre} codigo {proCodigo} tiene un precio {proPrecio} y su precio total es {proTotprecio} ha sido ingresado correctamente")

        # pass
ingresoProducto(miProducto)
"""
