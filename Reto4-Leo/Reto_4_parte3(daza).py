...
def inforServicio(dicc_lectura, dicc_tarifa):  # ================= inquietud
    ...
    val_consumo = list(map(lambda x: x*dicc_tarifa['consumo'], metros))
    val_cargo_fijo = list(map(lambda x: 1*dicc_tarifa['cargo_basico'], metros))
    ...
    lista_resultado = []
    lista_resultado_1 = []
    for x in cfijo_Est:  # lista tuplas valores-estrato
        for p, y in enumerate(subsidios):  # lista con tuplas estrato-subsidio
            if x[0] in subsidios[p]:
                lista_resultado.append(x[1]*y[1])

    for x in cvar_Est:  # lista tuplas valores-estrato
        for p, y in enumerate(subsidios):  # lista con tuplas estrato-subsidio
            if x[0] in subsidios[p]:
                lista_resultado_1.append(x[1]*y[1])
    ...
    valor_predios = list(map(lambda x, y: round(
        x+y, 2), lista_resultado, lista_resultado_1))
    #print("prueba loca:", valor_predios)
    total_predio = sum(valor_predios)
    # total_predio = reduce(lambda x, acum=0.00000000000001: (acum + x),0) tercer argumento valor inicial??
    # ============== inquietud
